...
python -m lerobot.async_inference.robot_client \
    ...
    --aggregate_fn_name=auto \ # CLIENT: resolves to the right overlap strategy for the checkpoint
...
- `aggregate_fn_name=auto` resolves to `latest_only` for checkpoints trained with action-prefix conditioning, and to `weighted_average` otherwise.
- TRTC-style checkpoints (`use_action_prefix_conditioning=true`) require `aggregate_fn_name=latest_only` and `actions_per_chunk == chunk_size`.
- For TRTC checkpoints, the server keeps the previous raw/model-space chunk and computes the prefix delay from the measured action-generation latency in control steps.
- Relative-action checkpoints are supported. The client still receives postprocessed absolute robot actions.
