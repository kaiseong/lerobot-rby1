...
def _import_zmq_dependencies():
    try:
        ...
        raise ImportError(
            "The 'pi05_thor' backend requires both 'msgpack' and 'pyzmq' to be installed."
        ) from exc
...
def normalize_zmq_server_address(server_address: str) -> str:
    ...
    if not sep or not host or not port:
        raise ValueError(
            f"pi05_thor backend expects server_address in 'host:port' format, got {server_address!r}"
        )
...
def validate_pi05_robot_compatibility(
    ...
    *,
    ...
    if action_keys != PI05_ACTION_KEYS:
        raise ValueError(
            "The 'pi05_thor' backend currently supports only "
    ...
    if missing_keys:
        raise ValueError(
            "Robot observation features are missing keys required by the pi05_thor backend: "
            f"{missing_keys}"
        )
