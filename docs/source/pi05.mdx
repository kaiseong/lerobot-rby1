...
π₀.₅ also supports **relative action training**, where the model predicts offsets from the
current robot state instead of absolute joint values. This mirrors OpenPI's delta-action
transform:

- preprocessing: `absolute_action -> relative_action = action - state`
- normalization: quantiles or mean/std on the **relative-action distribution**
- postprocessing: `relative_action -> absolute_action = relative + state`

To enable it:

First recompute your dataset stats in relative space:
...
- `relative_exclude_joints` keeps selected action dimensions absolute.
- the training pipeline recomputes chunk-level relative-action stats for the action normalizer.
- if you resume from pretrained processors, LeRobot rebuilds the processors from the current policy config so the relative transforms are not skipped.

### Training-Time RTC / Action-Prefix Conditioning

To train π₀.₅ with training-time real-time chunking (TRTC), enable action-prefix conditioning and leave overlap between `chunk_size` and `n_action_steps`:
...
- `action_prefix_length` is the maximum delay, in control steps, sampled during training.
- `n_action_steps` must be strictly smaller than `chunk_size` when action-prefix conditioning is enabled.
- `rtc_config.enabled` and `use_action_prefix_conditioning` are mutually exclusive: the former is inference-time RTC, the latter is training-time RTC.
- Internally, π₀.₅ samples from noise at flow time `1` toward actions at flow time `0`, so TRTC prefixes are injected as clean actions with prefix flow time `0`.

You can combine **relative actions + TRTC**:
...
lerobot-train \
    ...
    --policy.action_prefix_length=4
```

With this setup, TRTC prefix conditioning happens in model space on normalized relative actions,
while the robot still receives absolute actions after postprocessing.

When serving a TRTC checkpoint with [async inference](./async), set `actions_per_chunk` to the full `chunk_size` and use `aggregate_fn_name=auto` (or `latest_only` explicitly). The policy server will reuse the overlap from the previous raw action chunk and condition the next chunk on the actual async delay.
