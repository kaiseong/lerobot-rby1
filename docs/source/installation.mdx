...
This guide uses `conda` (via miniforge) to manage environments (recommended). If you prefer another environment manager (e.g. `uv`, `venv`), ensure you have Python >=3.12, support PyTorch >=2.10, and ffmpeg installed with the `libsvtav1` encoder, then skip ahead to [Environment Setup](#step-2-environment-setup).
