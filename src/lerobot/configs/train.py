...
class TrainPipelineConfig(HubMixin):
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            and self.sample_weighting is not None
            and self.sample_weighting.type == "rabc"
        ):
            raise NotImplementedError("RA-BC is not currently supported with dataset.sources mixed training.")
        ...
        if (
            self.sample_weighting is not None
            and self.sample_weighting.type == "rabc"
            and not self.sample_weighting.progress_path
        ):
            if self.dataset.root:
                self.sample_weighting.progress_path = str(Path(self.dataset.root) / "sarm_progress.parquet")
            else:
                self.sample_weighting.progress_path = (
                    f"hf://datasets/{self.dataset.repo_id}/sarm_progress.parquet"
                )
