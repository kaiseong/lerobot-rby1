...
- Relative-action checkpoints are supported. The server keeps the previous raw/model-space chunk for TRTC conditioning, while the client still receives postprocessed absolute robot actions.
